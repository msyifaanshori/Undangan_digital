...
export default function AdminDashboard() {
  // ==========================================
  // 1. STATE UNTUK SISTEM LOGIN
  // ==========================================
  const [isLoggedIn, setIsLoggedIn] = useState(false);
  const [passwordInput, setPasswordInput] = useState("");

  // ==========================================
  // 2. STATE UNTUK DATA DASHBOARD (Sama seperti sebelumnya)
  // ==========================================
  const [daftarUndangan, setDaftarUndangan] = useState([]);
  const [loading, setLoading] = useState(true);
  const [formData, setFormData] = useState({
    slug: "", nama_klien: "", nama_pria: "", ortu_pria: "",
    nama_wanita: "", ortu_wanita: "", tanggal_acara: "",
    tempat_akad: "", tempat_resepsi: ""
  });

  // Cek apakah admin sudah login sebelumnya (agar tidak perlu login terus saat di-refresh)
  useEffect(() => {
  ...
  const handleLogin = (e) => {
    e.preventDefault();
    // 🔐 GANTI "syifa2026" DENGAN PASSWORD RAHASIAMU SENDIRI
  ...
  // ==========================================
  // 3. FUNGSI DATABASE (Sama seperti sebelumnya)
  // ==========================================
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      ...
      setFormData({ slug: "", nama_klien: "", nama_pria: "", ortu_pria: "", nama_wanita: "", ortu_wanita: "", tanggal_acara: "", tempat_akad: "", tempat_resepsi: "" });
      ambilData();
    }
  };
  ...
  // JIKA BELUM LOGIN: Tampilkan Halaman Gembok
  if (!isLoggedIn) {
    return (
        ...
        <div className="bg-white p-8 rounded-2xl shadow-lg max-w-sm w-full border border-slate-200 text-center">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <input 
              ...
              className="w-full p-3 border rounded-lg focus:outline-blue-500 text-center text-lg tracking-widest"
              required 
            />
            <button type="submit" className="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-3 rounded-lg transition-all">
              Buka Kunci
            </button>
          </form>
        </div>
  ...
  // JIKA SUDAH LOGIN: Tampilkan Dashboard Asli
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 p-6 font-sans">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8 bg-white p-4 rounded-xl shadow-sm border border-slate-200">
          ...
          <button onClick={handleLogout} className="text-sm bg-red-100 text-red-600 hover:bg-red-200 px-4 py-2 rounded-lg font-bold transition-all">
            Logout
          </button>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* KIRI: FORM INPUT UNDANGAN BARU */}
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 lg:col-span-1">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              ...
              <button type="submit" className="w-full bg-slate-900 hover:bg-slate-800 text-white font-semibold py-3 rounded-lg shadow transition-all">
                ➕ Aktifkan Undangan
              </button>
            </form>
          </div>

          {/* KANAN: DAFTAR UNDANGAN YANG SEDANG AKTIF */}
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 lg:col-span-2">
            <h2 className="text-xl font-bold text-slate-950 mb-4">📋 Daftar Undangan Aktif</h2>
            {loading ? (
              <p className="text-sm text-gray-400">Sedang memuat data...</p>
            ) : daftarUndangan.length === 0 ? (
              <p className="text-sm text-gray-400">Belum ada undangan yang dibuat.</p>
            ) : (
                  ...
                  <tbody className="divide-y">
                    {daftarUndangan.map((item) => (
                      <tr key={item.id} className="hover:bg-slate-50">
                        ...
                        <td className="py-3">
                          <span className="bg-green-100 text-green-800 px-2.5 py-1 rounded-full font-semibold">Aktif</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
